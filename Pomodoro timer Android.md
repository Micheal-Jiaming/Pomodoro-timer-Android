...
│       ├── icons*\               icon sets fdroidserver generates per density
...
> **Status: built and committed, but not yet live.** GitHub Pages is enabled and configured
> for `docs/` on `main`, and the repository content is committed — but the commit carrying it
> has not been pushed, because the workspace quality gate refuses the push while its review
> markers are stale. Until `main` is pushed, that URL returns 404. One `git push origin main`
> makes it live within a minute or two. Nothing else is outstanding.
